...
bool gameplay = true; //tryb rozgrywki/tryb swobodnej kamery
bool wireframeMode = false; //tryb wireframe

glm::vec3 Position = glm::vec3(0.0f, 2.f, 5.5f); //pozycja kamery     
...
int main()
{
    ...
    //GENEROWANIE OBJEKTÓW GRAVELU=====================================================================================
    Shader gravelShader("grassShader.vert", "grassShader.frag");

    VAO gravelVAO;
    gravelVAO.Bind();  //obiektowka do vao.cpp

    VBO gravelVBO(gravelVertices, sizeof(gravelVertices)); // Generuje VBO
    EBO gravelEBO(gravelIndices, sizeof(gravelIndices)); // Generuje EBO

    //Przypisuje VBO takie jak kordyynaty czy kolory do VAO
    gravelVAO.LinkAttrib(gravelVBO, 0, 3, GL_FLOAT, 11 * sizeof(float), (void*)0);                    //vertex
    gravelVAO.LinkAttrib(gravelVBO, 1, 3, GL_FLOAT, 11 * sizeof(float), (void*)(3 * sizeof(float)));  //kolory
    gravelVAO.LinkAttrib(gravelVBO, 2, 2, GL_FLOAT, 11 * sizeof(float), (void*)(6 * sizeof(float)));  //textury
    gravelVAO.LinkAttrib(gravelVBO, 3, 3, GL_FLOAT, 11 * sizeof(float), (void*)(8 * sizeof(float)));  //normalsy
    ...
    Shader lampShader("lampsShader.vert", "lampsShader.frag");
    VAO lampVAO;
    lampVAO.Bind();
    ...
    Shader carShader("lightShader.vert", "lightShader.frag");
    VAO carVAO;
    carVAO.Bind();
    ...
    //GENEROWANIE OBJEKTÓW PUNKTU SPEEDA =====================================================================================

    Shader pointShader("pointShader.vert", "pointShader.frag");

    VAO pointVAO;
    pointVAO.Bind();

    VBO pointVBO(pointVertices, sizeof(pointVertices));
    EBO pointEBO(pointIndices, sizeof(pointIndices));
    ...
    glm::vec3 gravelPos = glm::vec3(0.0f, 0.0f, 0.0f);
    glm::mat4 gravelModel = glm::mat4(1.0f);
    gravelModel = glm::scale(gravelModel, glm::vec3(1.0f, 1.0f, 1.0f));
    gravelModel = glm::translate(gravelModel, gravelPos);
    ...
    gravelShader.use();
    glUniformMatrix4fv(glGetUniformLocation(gravelShader.ID, "model"), 1, GL_FALSE, glm::value_ptr(gravelModel));
    glUniform4f(glGetUniformLocation(gravelShader.ID, "lightColor"), lightColor.x, lightColor.y, lightColor.z, lightColor.w);
    glUniform3f(glGetUniformLocation(gravelShader.ID, "lightPos"), lightPos.x, lightPos.y, lightPos.z);
    glUniform3f(glGetUniformLocation(gravelShader.ID, "lightPos2"), lightPos2.x, lightPos2.y, lightPos2.z);
    glUniform4f(glGetUniformLocation(gravelShader.ID, "lightColor2"), lightColor2.x, lightColor2.y, lightColor2.z, lightColor2.w);
    buildingShader.use();
    ...
    pointShader.use();
    glUniformMatrix4fv(glGetUniformLocation(pointShader.ID, "model"), 1, GL_FALSE, glm::value_ptr(pointModel));
    glUniform4f(glGetUniformLocation(pointShader.ID, "lightColor"), lightColor.x, lightColor.y, lightColor.z, lightColor.w);
    glUniform3f(glGetUniformLocation(pointShader.ID, "lightPos"), lightPos.x, lightPos.y, lightPos.z);
    ...
    //TEXTURA3-POCZĄTEK
    unsigned int texture3 = loadTexture("gravel.png");
    ...
    cout << "A - LEWO" << endl;
    cout << "D - PRAWO" << endl;
    cout << "LEWY SHIFT - PRZYSPIESZENIE" << endl;
    cout << "M - TRYB SWOBODNEJ KAMERY" << endl;
    cout << "N - POWROT DO TRYBU GRACZA" << endl;
    cout << "L - TRYB POLIGONOW" << endl;
    cout << "K - POWROT DO TRYBU GRACZA" << endl << endl;
    // Główna pętla
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (endGame == false) {
            ...
            if ((glfwGetKey(window, GLFW_KEY_M) == GLFW_PRESS) && !mPressed) { //Tryb obserwatora
                RememberedPosition = Position;
                gameplay = false;
                mPressed = true;
            }
            if ((glfwGetKey(window, GLFW_KEY_N) == GLFW_PRESS) && mPressed) { //Tryb gracza
            ...
            for (int i = 0; i < 1; i++) {
                ...
                {
                    glm::vec3 gravelPos = glm::vec3(0.0f + (i * 2), 0.001f, 0.0f + (j * 2));
                    glm::mat4 gravelModel = glm::mat4(1.0f);
                    gravelModel = glm::scale(gravelModel, glm::vec3(5.0f, 5.0f, 5.0f));
                    gravelModel = glm::translate(gravelModel, gravelPos);
                    ...
                    //planksSpec.Bind();
                    gravelVAO.Bind(); //Przypisz VAO ¿eby OpenGL go u¿ywa³
                    glDrawElements(GL_TRIANGLES, sizeof(gravelIndices) / sizeof(int), GL_UNSIGNED_INT, 0); //Narysuj trójk¹t u¿ywaj¹c GL_TRIANGLE
                }
            ...
            for (int j = grass_height_min+2; j < grass_height_max+2; j++)
            {
                glm::vec3 buildingPos = glm::vec3(5.0f, 0.0f, 0.0f + (j * 6));
                glm::mat4 buildingModel = glm::mat4(1.0f);
            ...
            pointModel = glm::translate(pointModel, pointPos);
            pointModel = glm::scale(pointModel, glm::vec3(0.2f, 0.2f, 0.2f));
            pointModel = glm::rotate(pointModel, glm::radians(pointRot), glm::vec3(0.0f, 1.0f, 0.0f));   //rotacja
            ...
            glDrawElements(GL_TRIANGLES, sizeof(pointIndices) / sizeof(int), GL_UNSIGNED_INT, 0);
...
void setDefaults() {
    Position = glm::vec3(0.0f, 2.f, 5.5f); //pozycja kamery   
