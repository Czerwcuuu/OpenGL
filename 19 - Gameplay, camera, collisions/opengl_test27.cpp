...
//TODO:
//
//Obracanie kamery względem postaci (not sure) - DONE
//Generowanie 'punktow' w losowych miejscach - DONE
//Generowanie budynkow w losowych miejscach (z odstepem od siebie)
//Kolizja - DONE
//Licznik punktów (GUI lub w pasku) - PRAWIE DONE
//Jakieś lampy? (wykorzystanie świateł)
//Jeden element animowany - samochód, drugi już mamy (postać) - DONE
//Gameplay - DONE
...
int main()
{
    ...
    glm::vec3 carPos = glm::vec3(0.5f, 0.5f, -10.f);
    ...
    while (!glfwWindowShouldClose(window))
    {
        if (endGame == false) {
            ...
            //Powoduje ze kamera patrzy sie we wlasciwym kierunku
            view = glm::lookAt(Position, modelPos + Orientation, Up);
            //Dodaje perspektywe do sceny
            projection = glm::perspective(glm::radians(FOVdeg), (float)(SCR_WIDTH / SCR_HEIGHT), nearPlane, farPlane);
            //Eksportuje kamere do vertex shadera
            cameraMatrix = projection * view;
            ...
            skyboxShader.use();
            view = glm::mat4(glm::mat3(glm::lookAt(Position, modelPos + Orientation, Up)));
